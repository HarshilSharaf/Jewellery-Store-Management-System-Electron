...
BEGIN
  DECLARE current_count DECIMAL(14, 3);
  DECLARE previous_count DECIMAL(14, 3);
  DECLARE percent_increase DOUBLE(5, 2);
  DECLARE total DECIMAL(14, 3);

  SET time_zone = 'SYSTEM';

  SELECT COALESCE(SUM(netWeight), 0) INTO current_count
    FROM products
   WHERE createdAt >= DATE_SUB(NOW(), INTERVAL 6 MONTH)
     AND isSold = 0
     AND deletedAt IS NULL;

  SELECT COALESCE(SUM(netWeight), 0) INTO previous_count
    FROM products
   WHERE createdAt < DATE_SUB(NOW(), INTERVAL 6 MONTH)
     AND deletedAt IS NULL;
  ...
  SELECT COALESCE(SUM(netWeight), 0) INTO total
    FROM products
   WHERE isSold = 0
     AND deletedAt IS NULL;
